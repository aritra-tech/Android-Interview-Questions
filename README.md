...
- **What is `View` in Android?** 
  - [Learn from here](https://developer.android.com/reference/android/view/View)
    <br>
...
- **How do you implement ViewModel in Android and what are it's importance?**

    - Firstly, add required dependencies: [Get from here](https://developer.android.com/jetpack/androidx/releases/lifecycle)
    - We create a class, for ex. ```MainViewModel.kt```
    - Then we mention its type as ```: ViewModel()```.
    - Now going back in activity we make a lateinit var (ViewModel instance), ex. 'mainViewModel'
    - Up next we initialize it using ViewModelProvider.
    - It takes owner name at first and then the name of ViewModel class.
    - Ex. ```mainViewModel = ViewModelProvider(this)[MainViewModel::class.java]```
    - Now we can call any function inside the viewModel or observe any LiveData or MutableLiveData as well.
    - [Learn More](https://developer.android.com/topic/libraries/architecture/viewmodel)
...
- **What is data class in Kotlin and how it's different from Java?**
    ...
    Kotlin data class is similar to Java class, with some advantages Kotlin leads the way. Class is a blueprint or template of an object that is to be created.
    For ex. we have some properties (attributes) of a user:
     - Name
     - Address
     - ID
     - Phone
    ...
    We can't just store seperate strings, so to group them into one class (entity) we user data class in Kotlin.
    [More about data class](https://kotlinlang.org/docs/data-classes.html)
    [Similar resource for differnces between Kotlin data class and Java class](https://www.geeksforgeeks.org/difference-between-java-and-kotlin-in-android-with-examples/)